...
export const speedLimiter = slowDown({
  ...
  // express-slow-down v2+ expects delayMs as a function.
...
export const sanitizeInput = (req: Request, res: Response, next: NextFunction) => {
  // Never mutate password fields before auth validation
  // (backend Joi password regex is strict about special characters)
  ...
  if (req.body) {
    // Sanitize object but preserve password-like fields as-is
    const sanitizeObjectPreservingPasswords = (obj: any): any => {
      if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
        ...
        for (const [key, value] of Object.entries(obj)) {
          if (passwordFields.has(key)) {
            ...
            // Reuse existing sanitizer for nested content
            sanitized[key] = sanitizeObject(value);
          }
        }
        return sanitized;
    ...
    req.body = sanitizeObjectPreservingPasswords(req.body);
  }
  if (req.query) {
