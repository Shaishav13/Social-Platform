...
router.delete('/posts/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const deleted = await PostModel.deletePost(id, userId!, isAdmin);
...
router.post('/upload', authenticateToken, upload.fields([{ name: 'files', maxCount: 5 }, { name: 'media', maxCount: 5 }]), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const response: any = {
      ...
      urls: uploadResults.map((u: any) => u.url)
    };
